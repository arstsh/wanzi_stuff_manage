...
export const itemService = {
    async create(data) {
        ...
        if (skuExists)
            throw new ConflictError('SKU 已存在');
        if (nameExists)
            throw new ConflictError('货品名称已存在');
    ...
    async list(options) {
        ...
        if (options.page < 1)
            throw new BadRequestError('page 必须 ≥ 1');
        if (options.pageSize < 1 || options.pageSize > 100)
            throw new BadRequestError('pageSize 范围 1-100');
    ...
    async adjustStock(itemId, type, quantity, note) {
    ...
    async getLogs(itemId, page, pageSize) {
