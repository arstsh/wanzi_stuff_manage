...
function signAccessToken(payload: { userId: number; username: string; role: string }) {
...
export const authController = {
  ...
  login: asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    ...
    const response: ApiResponse<{ accessToken: string; user: { id: number; username: string; role: string } }> = {
      success: true,
      data: {
        accessToken,
  ...
  refresh: asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    ...
    try {
      payload = jwt.verify(token, config.REFRESH_TOKEN_SECRET) as { userId: number };
    } catch {
  ...
  me: asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    const response: ApiResponse<{ userId: number; username: string; role: string }> = {
      success: true,
      data: {
